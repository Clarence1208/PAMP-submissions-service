...
from app.domains.submissions.submissions_models import Submission, SimilarityStatus
...
class DetectionIntegrationService:
    ...
    def process_submission_similarities(self, submission: Submission) -> List[str]:
        ...
        try:
            ...
            other_submissions = [
                ...
                if s.id != submission.id and s.status.value == "completed"
            ]
            ...
            for other_submission in other_submissions:
                try:
                    ...
                    if self.similarity_repository.check_existing_comparison(
                        submission.id, other_submission.id
                    ):
                    ...
                    logger.error(f"Failed to process comparison between {submission.id} and {other_submission.id}: {str(e)}")
                    continue
    ...
    def _process_single_comparison(
        ...
        try:
            ...
            self.similarity_repository.update_status(
                ...
                for file_path in repo1_compatible_files:
                    ...
                    for file_path2 in repo2_compatible_files:
                        ...
                        react_flow_data = self.visualization_service.generate_react_flow_ast(
                            content1, content2,
                            file_path.name, file_path2.name, "elk"  # Always use ELK
                        )
    ...
    def _detect_shared_blocks_multifile(
        ...
        logger.info(f"Optimized comparison: {successful_comparisons}/{len(file_pairs)} smart file pairs had similarities, {len(all_shared_blocks)} total shared blocks found")
        return result
    ...
    def _preprocess_files(self, files: List[Path], repo_path: Path, repo_name: str) -> Dict[str, List[Dict]]:
        ...
        logger.info(f"Pre-processed {sum(len(files) for files in processed_by_language.values())} files from {repo_name} across {len(processed_by_language)} languages")
